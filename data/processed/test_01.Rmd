...
cat("\n📋 Columnas en común:", length(common_cols), "\n\n")

# ---- Combina para comparación ----
...
cat("\n✅ Done. Correlations and scatterplots printed above.\n")
...
# --- Helper: calcula alpha DFA sobre una serie (regresión log-log) ---
dfa_alpha <- function(x) {
  x <- as.numeric(x)
  x <- x[is.finite(x)]
  if (length(x) < 200) return(NA_real_)  # mínima longitud sensata
...
# Ventana: ±5 s alrededor de la transición entre objetos (t_targ1StartTime==0)
...
rows <- lapply(ids, function(id) {
  ...
  # Elige la señal: euc_vel o ang_vel
