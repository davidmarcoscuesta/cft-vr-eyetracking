...
```{r}
results <- pmap_dfr(
  as.list(as.data.frame(grid)),
  ~ evaluate_combo(win_ms = ..1, step_ms = ..2, range_ms = ..3, k = ..4)
)

# Guardamos resultados brutos por si queremos inspeccionar luego
dir.create("results", showWarnings = FALSE, recursive = TRUE)
readr::write_csv(results, "results/window_param_sweep.csv")

results %>% arrange(desc(coverage), desc(euc_d)) %>% head(10)
```
